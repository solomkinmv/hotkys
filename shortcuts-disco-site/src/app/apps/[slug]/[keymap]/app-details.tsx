...
import { modifierSymbols } from "@/lib/model/internal/modifiers";
...
import { ShortcutComment } from "@/components/ui/shortcut-comment";
          ...
          <ListItem key={hotkey.title + idx}>
            ...
            <span className="text-right text-gray-900">
              {hotkey.sequence.length > 0 && (
                <KeyboardBadge
                  tokens={generateFormattedHotkeyTokens(hotkey)}
                  className="ml-2"
                />
              )}
              {hotkey.comment && hotkey.comment.length > 0 && (
                <ShortcutComment comment={hotkey.comment} className="ml-2" />
              )}
            </span>
          </ListItem>
        ))}
...
function generateFormattedHotkeyTokens(shortcut: SectionShortcut): string[] {
  return shortcut.sequence.flatMap((atomicShortcut) => {
    const modifiersText: string[] = atomicShortcut.modifiers
      .map((modifier) => modifierSymbols.get(modifier))
      .filter((value) => value !== undefined) as string[];
    return [
      ...modifiersText,
      overrideSymbolIfPossible(atomicShortcut.base),
      " ",
    ];
  });
}
